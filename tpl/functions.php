...
function bootstrap3_navbar()
{
    ...
    if (bootstrap3_conf('showNavbar') === 'logged' && !$_SERVER['REMOTE_USER']) {
        return false;
    }
...
function bootstrap3_searchform()
{
    ...
    if (!bootstrap3_conf('showSearchForm')) {
        return false;
    }
...
function bootstrap3_conf($key, $default = false)
{
    ...
    switch ($key) {
        ...
        case 'tocCollapseOnScroll':
            if (bootstrap3_conf('tocLayout') !== 'default') {
                return false;
            }
...
function bootstrap3_google_analytics()
{
    ...
    if (!bootstrap3_conf('useGoogleAnalytics')) {
        return false;
    }

    if (!$google_analitycs_id = bootstrap3_conf('googleAnalyticsTrackID')) {
        return false;
    }

    if (bootstrap3_conf('googleAnalyticsNoTrackAdmin') && $INFO['isadmin']) {
        return false;
    }

    if (bootstrap3_conf('googleAnalyticsNoTrackUsers') && isset($_SERVER['REMOTE_USER'])) {
        return false;
    }

    if (tpl_getConf('googleAnalyticsNoTrackPages')) {
        if (preg_match_all(bootstrap3_conf('googleAnalyticsNoTrackPages'), $ID)) {
            return false;
        }
...
function bootstrap3_theme_by_namespace()
{
    ...
    if (!bootstrap3_conf('themeByNamespace')) {
        return array();
    }
...
function bootstrap3_classes()
{
    ...
    if (!bootstrap3_conf('tableFullWidth')) {
